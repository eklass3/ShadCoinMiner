...
class Requests:
    ...
    @staticmethod
    def FetchPrevHash():
        response = requests.post("http://localhost:8000/getBlocks")  # Collect previous blocks
    ...
    def SaveNewBlock(nonce, name, hash):
        response = requests.post("http://localhost:8000/createBlock", json={'nonce': nonce, 'name': name, 'hash': hash})
